	...
	return (
		<main className="w-full h-full bg-orange-100 py-10">
			...
			<div className="flex my-5">
				<Link
					href={"/blog/add"}
					className="text-orange-500 md:w-1/6 sm:w-2/4 text-center rounded-md p-2 m-auto bg-white font-semibold"
				>
					ブログ新規作成
				</Link>
			</div>
					...
					>
						<div className="flex items-center my-3">
							<Link href={`/blog/${post.id}`} className="mr-auto">
								<h1 className="mr-auto font-semibold text-2xl">{post.title}</h1>
							</Link>
							<Link
								href={`/blog/edit/${post.id}`}
								className="px-4 py-1 text-center text-xl bg-orange-500 text-white rounded-md hover:bg-orange-600"
							>
								編集
							</Link>
						</div>
